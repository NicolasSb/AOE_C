...
static void test_Building_initiation_Case_HOUSE(void **state){
...
static void test_Building_initiation_Case_BARRACKS(void **state){
...
static void test_Building_initiation_Case_CITYHALL(void **state){
...
static void test_Building_initiation_Case_TOWER(void **state){
...
static void test_freeBuilding_Handles_initialisedBuilding (void **state)
{
    Building *b = *state;
...
static void test_freeBuilding_Handles_uninitialisedBuilding (void **state)
{
    Building *b = *state;
    int res = freeBuilding(b);
...
static void test_setActionFlag_Handles_initialisedBuilding(void ** state)
{
    int res;
    Building *b = *state;
...
static void test_setActionFlag_Handles_uninitialisedBuilding(void ** state)
{
    int res;
    Building *b = *state;
    setActionFlag(b, CONSTRUCT_P);
...
static void test_setTarget(void ** state)
{
    Building *b = *state;
    setTargetB(b, (void*)b);
    assert_ptr_equal((void*)b, b->target);
}

static void test_getHP(void ** state)
{
    Building *b = *state;
    int res = getHP(b);
    assert_int_equal(b->hp, res);
}


static void test_getHP_uninitialisedB(void ** state)
{
    Building *b = *state;
    int res = getHP(b);
    assert_int_equal(-1, res);
...
static int test_all_buildings()
{
    const struct CMUnitTest test_Building[] ={
        ...
        cmocka_unit_test_setup_teardown(test_setTarget,setup_Building, teardown),
        cmocka_unit_test_setup_teardown(test_getHP,setup_Building,teardown),
        cmocka_unit_test(test_getHP_uninitialisedB)
    };
